...
class Repository:
    def __init__(self, name, description, language, url, stargazer_count, is_private, topics, commit_date, licenseinfo):
        self.name = name
...
class GitHubGQL:
    ...
    def get_user_starred_by_username(self, username: str, after: str = '', topic_stargazer_count_limit: int = 0):
        ...
        for repo in result['user']['starredRepositories']['nodes']:
            ...
            topics = [tag['topic']['name'] for tag in repo['repositoryTopics']['nodes'] if
                      tag['topic']['stargazerCount'] > topic_stargazer_count_limit]
            commit_date = repo['pushedAt']
            try:
                if repo['licenseInfo'] is not None:
                   licenseinfo = repo ['licenseInfo']['name']
                else:
                    licenseinfo = "❌"
            except NameError:
                licenseinfo = "❌"
            items.append(Repository(name, description, language, url, stargazer_count, is_private, topics, commit_date,
                                    licenseinfo))
        if has_next:
